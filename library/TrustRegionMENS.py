...
class TrustRegionMENS(object):
    ...
    def run_TR_algorithm(self):
        ...
        def Flrich_bounds(model,i,j,k):
            if model.y[i,j,k] ==1:
                return (0.0, model._rich_data.at[i,'F'])
            else:
                return (0.0,50)
        ...
        def Flean_init(model,i,j,k):
            if k == (model.nstages+1):
                return Constraint.Skip
            elif  y(i,j,k)==1 and model.l_exist[j,k] == 1:
                return (model.M[i,j,k].value/(model.cl[j,k].value-model.cl[j,k+1].value))
            else:
                return 0   
            
        model.Flrich = Var(model.i,model.j,model.k, within = NonNegativeReals)
        ...
        def FlowLean_(model,j,k):
            if k == (model.nstages+1):
                return Constraint.Skip
            elif model.l_exist[j,k] == 1 and model.y[i,j,k] == 1:
