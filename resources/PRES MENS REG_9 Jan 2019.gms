...
2.1        150         150      0.2       0.00        170
2.2        150         150      0.2       0.00        170
3.1        100         20       0.2       0.38        0.0
3.2        100         20       0.2       0.38        0.0
4.1        180         50       0.2       1.00        0.0
4.2        175         45       0.2       1.20        0.0
5.1        140         70       0.2       2.00        0.0
5.2        140         60       0.2       2.20        0.0
6.1        150         45       0.2       6.50        0.0
6.2        150         35       0.2       6.00        0.0
7.1        145         55       0.2       8.50        0.0
7.2        145         55       0.2       8.50        0.0;

TABLE COLDS(Jc,P,DATA) Cold streams data
          TIN        TOUT        H          F          COST
1.1        20         100       0.2        0.38        0
1.2        20         100       0.2        0.38        0
2.1        30         95        0.2        5.000       0
2.2        30         95        0.2        5.000       0
3.1        40         90        0.2        3.000       0
3.2        40         90        0.2        3.000       0
4.1        0          10        0.2        0.00        1.3
4.2        0          10        0.2        0.00        1.3;
...
AC('1')=120000;
AC('2')=120000;
AC('3')=000; AZ('1',P)=121460;  KW=0.02; AF=.225; W=.1;
EMAC=.00001;  INTRATE =0.3;  OrigAOC=168050;
...
gamma(ih,jc,P) = .000000000000000000000000001;
...
STAGE_HEAT_HOT1(Ih,p,Kh)   STAGE HEAT BALANCE OF HOT PROCESS STREAM HPS(I)
STAGE_HEAT_COLD1(Jc,p,Kh)   STAGE HEAT BALANCE OF COLD PROCESS STREAM CPS(I)
...
LOG_Q_HPS_CPS1(Ih,Jc,p,J,Kh) LOGICAL CONSTRAINT ON HEAT EXCHANGED BETWEEN HPS(I) AND CPS(J)
LOG_Q_HPS_CUT1(Ih,Jc,p,Kh) LOGICAL CONSTRAINT ON HEAT EXCHANGED BETWEEN HPS(I) AND CUT(J)
LOG_Q_HUT_CPS1(Ih,Jc,p,Kh) LOGICAL CONSTRAINT ON HEAT EXCHANGED BETWEEN HUT(I) AND CPS(J)
...
TOTAL_MASS_LEAN(J,P)$LPS(J,P).. L(J,P)*(LEAN(J,P,'COUT')-LEAN(J,P,'CIN')) =e=SUM((I,K)$(MATCH(I,J,P,K)),M(I,J,P,K));

TOTAL_MASS_REG(R,P)$RES(R,P)..V(R,P)*(ZEAN(R,P,'COUT')-ZEAN(R,P,'CIN'))=e=SUM((I,K)$(MATCH(I,'3',P,K)),M(I,'3',P,K));

*stream overall enthalpy balance


TOTAL_HEAT_HOT1(Ih,p)$(HPS(Ih,p))..  HOTS(Ih,p,'F')*(HOTS(Ih,p,'TIN')-HOTS(Ih,p,'TOUT')) =e=
                              SUM((Jc,Kh)$(matchh (ih,jc,p,kh)), Q(Ih,Jc,p,Kh));

TOTAL_HEAT_COLD1(Jc,p)$(CPS(Jc,p)).. COLDS(Jc,p,'F')*(COLDS(Jc,p,'TOUT')-COLDS(Jc,p,'TIN')) =e=
...
STAGE_HEAT_HOT1(Ih,p,Kh)$(HPS(Ih,p) AND SUM(Jc,matchh (ih,jc,p,kh)))..
HOTS(Ih,p,'F')*(TH(Ih,p,Kh)-TH(Ih,p,Kh+1)) =e= SUM(Jc$matchh (ih,jc,p,kh), Q(Ih,Jc,p,Kh));

STAGE_HEAT_COLD1(Jc,p,Kh)$(CPS(Jc,p) AND SUM(Ih,matchh (ih,jc,p,kh)))..
...
*FEASIBLE(I,J,K)$MATCH(I,J,K)..CR(I,K)-CL(J,K)=G=EMAC;
*===============================================================================

LOG_M_RPS_LPS(I,J,P,K)$(RPS(I,P) AND LPS(J,P) AND MATCH(I,J,P,K))..M(I,J,P,K) =L= MIN(RICH(I,P,'F')*(RICH(I,P,'CIN')-RICH(I,P,'COUT')),L(J,P)*(LEAN(J,P,'COUT')-LEAN(J,P,'CIN')))*Y(I,J,K);
*===============================================================================
*Calculation of exchanger driving forces between rich and lean streams

LOG_DC_RPS_LPS_RS(I,J,P,K)$(RPS(I,P) AND LPS(J,P) AND MATCH(I,J,P,K))..DC(I,J,P,K) =L= CR(I,P,K) - CL(J,P,K) + OMEGA(I,J,P)*(1-Y(I,J,K));
LOG_DC_RPS_LPS_RS1(I,J,P,K)$(RPS(I,P) AND LPS(J,P) AND MATCH(I,J,P,K))..DC(I,J,P,K ) =G= CR(I,P,K) - CL(J,P,K) - OMEGA(I,J,P)*(1-Y(I,J,K));
LOG_DC_RPS_LPS_LS(I,J,P,K)$(RPS(I,P) AND LPS(J,P) AND MATCH(I,J,P,K))..DC(I,J,P,K+1) =L= CR(I,P,K+1) - CL(J,P,K+1) + OMEGA(I,J,P)*(1-Y(I,J,K));
LOG_DC_RPS_LPS_LS1(I,J,P,K)$(RPS(I,P) AND LPS(J,P) AND MATCH(I,J,P,K))..DC(I,J,P,K+1) =G= CR(I,P,K+1) - CL(J,P,K+1) - OMEGA(I,J,P)*(1-Y(I,J,K));
...
LOG_Q_HPS_CPS1(Ih,Jc,p,J,Kh)$(HPS(Ih,p) AND CPS(Jc,p) AND matchh (ih,jc,p,kh))..
Q(Ih,Jc,p,Kh) =L= MIN(HOTS(Ih,p,'F')*(HOTS(Ih,p,'TIN')-HOTS(Ih,p,'TOUT')),COLDS(Jc,p,'F')*(COLDS(Jc,p,'TOUT')-COLDS(Jc,p,'TIN')))*Yh(Ih,Jc,Kh);

LOG_Q_HPS_CUT1(Ih,Jc,p,Kh)$(HPS(Ih,p) AND CUT(Jc,p) AND matchh (ih,jc,p,kh))..
Q(Ih,Jc,p,Kh) =L= HOTS(Ih,p,'F')*(HOTS(Ih,p,'TIN')-HOTS(Ih,p,'TOUT'))*Yh(Ih,Jc,Kh);

LOG_Q_HUT_CPS1(Ih,Jc,p,Kh)$(HUT(Ih,p) AND CPS(Jc,p) AND matchh (ih,jc,p,kh))..
...
*Capital cost terms for matches involving lean stream 1 which requires tray columns

*Capital cost terms for matches involving lean stream 3 and regenerating stream which requires continuous contact columns
+ AF*(ACH*MXR('1'))**D
...
DC.L(I,J,P,K)$(RPS(I,P) AND LPS(J,P) AND MATCH(I,J,P,K))=RICH(I,P,'CIN')-LEAN(J,P,'CIN') ;
DC.L(I,J,P,K+1)$(RPS(I,P) AND LPS(J,P) AND MATCH(I,J,P,K))=RICH(I,P,'CIN')-LEAN(J,P,'CIN');
DC.LO(I,J,P,K)$(RPS(I,P) AND LPS(J,P) AND MATCH(I,J,P,K))=EMAC;
DC.LO(I,J,P,K+1)$(RPS(I,P) AND LPS(J,P) AND MATCH(I,J,P,K))=EMAC;
DC.UP(I,J,P,K)$(RPS(I,P) AND LPS(J,P) AND MATCH(I,J,P,K))=1;
DC.UP(I,J,P,K+1)$(RPS(I,P) AND LPS(J,P) AND MATCH(I,J,P,K))=1;

CAPINVEST.LO= .1;
CAPINVEST.L= .1;
CAPINVEST.UP= 1000000;

Annualcost.LO= .1;
Annualcost.L= .1 ;
Annualcost.UP= 1000000;

L.L('1',P)=.1;  L.LO('1',P)=.01;  L.UP('1',P)=2.0;
L.L('2',P)=.1;  L.LO('2',P)=.01;  L.UP('2',P)=1.9;
L.L('3',P)=.1;  L.LO('3',P)=.0001;  L.UP('3',P)=20;
V.L('1',P)=.1;  V.LO('1',P)=.1;  V.UP('1',P)=15.25;
*m for regeneration kept constant

*Initialisations for M(I,J,K) between RPS(I) and LPS(J)
*$ONTEXT
M.L(I,J,P,K)$(RPS(I,P) AND MATCH(I,J,P,K))=(RICH(I,P,'F')*(RICH(I,P,'CIN')-RICH(I,P,'COUT')));
M.L(I,J,P,K)$(RPS(I,P) AND MATCH(I,J,P,K))=MIN(RICH(I,P,'F')*(RICH(I,P,'CIN')-RICH(I,P,'COUT')),L.L(J,P)*(LEAN(J,P,'COUT')-LEAN(J,P,'CIN')));

M.UP(I,J,P,K)$(RPS(I,P) AND LPS(J,P) AND MATCH(I,J,P,K))=MIN(RICH(I,P,'F')*(RICH(I,P,'CIN')-RICH(I,P,'COUT')),L.L(J,P)*(LEAN(J,P,'COUT')-LEAN(J,P,'CIN')));
M.UP(I,J,P,K)$(RPS(I,P) AND MATCH(I,J,P,K)) = RICH(I,P,'F')*(RICH(I,P,'CIN')-RICH(I,P,'COUT'));
*$offtext
...
Q.L(Ih,Jc,P,Kh)$(HPS(Ih,P) AND CPS(Jc,P) AND matchh (ih,jc,p,kh))=MIN(HOTS(Ih,p,'F')*(HOTS(Ih,P,'TIN')-HOTS(Ih,P,'TOUT')),COLDS(JC,P,'F')*(COLDS(Jc,P,'TOUT')-COLDS(Jc,P,'TIN')));
Q.L(Ih,Jc,P,Kh)$(HPS(Ih,P) AND CUT(Jc,P) AND matchh (ih,jc,p,kh)) = HOTS(Ih,p,'F')*(HOTS(Ih,P,'TIN')-HOTS(Ih,P,'TOUT'));
Q.L(Ih,Jc,P,Kh)$(HUT(Ih,P) AND CPS(Jc,P) AND matchh (ih,jc,p,kh))= COLDS(JC,P,'F')*(COLDS(Jc,P,'TOUT')-COLDS(Jc,P,'TIN'));

Q.UP(Ih,Jc,P,Kh)$(HPS(Ih,P) AND CPS(Jc,P) AND matchh (ih,jc,p,kh))=MIN(HOTS(Ih,p,'F')*(HOTS(Ih,P,'TIN')-HOTS(Ih,P,'TOUT')),COLDS(JC,P,'F')*(COLDS(Jc,P,'TOUT')-COLDS(Jc,P,'TIN')));
Q.UP(Ih,Jc,P,Kh)$(HPS(Ih,P) AND CUT(Jc,P) AND matchh (ih,jc,p,kh)) = HOTS(Ih,p,'F')*(HOTS(Ih,P,'TIN')-HOTS(Ih,P,'TOUT'));
Q.UP(Ih,Jc,P,Kh)$(HUT(Ih,P) AND CPS(Jc,P) AND matchh (ih,jc,p,kh))= COLDS(JC,P,'F')*(COLDS(Jc,P,'TOUT')-COLDS(Jc,P,'TIN'));
...
REGCOST=SUM((P),(DOP(P)/NOP)*V.l('1',P)*AZ('1',P));
...
AOC =SUM((J,P),(DOP(P)/NOP)*L.L(J,P)*AC(J))

*Flowrate of regenerating stream multiplied by cost per unit of regenerating stream
+SUM((P),(DOP(P)/NOP)*V.L('1',P)*AZ('1',P))

+SUM((P),(DOP(P)/NOP)* SUM((Ih,Jc,Kh),COLDS(Jc,p,'COST')*Q.L(Ih,Jc,p,Kh)))
+SUM((P),(DOP(P)/NOP)*SUM((Ih,Jc,Kh),HOTS(Ih,p,'COST')*Q.L(Ih,Jc,p,Kh))) ;
